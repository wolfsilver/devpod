...
func NewOpenVSCodeWebServer(extensions []string, settings string, userName string, host, port string, values map[string]config.OptionValue, log log.Logger) *OpenVSCodeWebServer {
	return &OpenVSCodeWebServer{
		values:     values,
...
type OpenVSCodeWebServer struct {
...
func (o *OpenVSCodeWebServer) InstallExtensions() error {
	// install extensions
...
func (o *OpenVSCodeWebServer) Install() error {
	location, err := prepareOpenVSCodeWebServerLocation(o.userName)
	if err != nil {
		return err
	}
...
func (o *OpenVSCodeWebServer) getReleaseUrl() string {
	var url string
	...
	if runtime.GOARCH == "arm64" {
		...
		if url == "" {
			url = fmt.Sprintf(DownloadArm64Template)
		}
	} else {
		url = Options.GetValue(o.values, DownloadAmd64Option)
		if url == "" {
			url = fmt.Sprintf(DownloadAmd64Template)
		}
	}
...
func (o *OpenVSCodeWebServer) installExtensions() error {
	...
	binaryPath := filepath.Join(location, "bin", "openvscodeweb-server")
...
func (o *OpenVSCodeWebServer) installSettings() error {
	if len(o.settings) == 0 {
		return nil
	}
...
func (o *OpenVSCodeWebServer) Start() error {
	location, err := prepareOpenVSCodeWebServerLocation(o.userName)
	if err != nil {
		return err
	}
	...
	binaryPath := filepath.Join(location, "bin", "code")
	_, err = os.Stat(binaryPath)
	if err != nil {
		return errors.Wrap(err, "find binary")
	}

	return single.Single("openvscodeweb.pid", func() (*exec.Cmd, error) {
		o.log.Infof("Starting openvscodeweb in background...")
		runCommand := fmt.Sprintf("%s serve-web --without-connection-token --host '%s' --port '%s'", binaryPath, o.host, o.port)
		args := []string{}
		if o.userName != "" {
			args = append(args, "su", o.userName, "-c", runCommand)
		} else {
...
func prepareOpenVSCodeWebServerLocation(userName string) (string, error) {
	var err error
	homeFolder := ""
	if userName != "" {
	...
	folder := filepath.Join(homeFolder, ".openvscodeweb-server")
